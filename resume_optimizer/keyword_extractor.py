...
class KeywordExtractor:
    ...
    def _calculate_word_scores(self, phrase_list):
        ...
        for phrase in phrase_list:
            ...
            multi_word = min(2, len(list(filter(lambda x: not is_numeric(x), phrase))))
            for word in phrase:
...
def main(file_path):
    ...
    with open(file_path) as file:
        #content = unicode(file.read(), errors='ignore')   #Old version
        content = str(file.read(), errors='ignore')
